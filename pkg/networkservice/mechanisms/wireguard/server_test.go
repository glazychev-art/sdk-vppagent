...
func TestWireguardServer(t *testing.T) {
	// Turn off log output
	...
	suite.Run(t, checkvppagentmechanism.NewServerSuite(
		...
		func(t *testing.T, conf *configurator.Config) {
			...
			assert.Equal(t, tp.dstIP, wireguardInterface.GetSrcAddr())
			assert.Equal(t, uint32(tp.dstPort), wireguardInterface.GetPort())
			...
			assert.Equal(t, uint32(tp.srcPort), wireguardPeer.GetPort())
			assert.Equal(t, tp.srcIP, wireguardPeer.GetEndpoint())
